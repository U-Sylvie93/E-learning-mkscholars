...
class EntranceExamInstitution extends Model
{
    use HasFactory;
    ...
    {
        static::saving(function (EntranceExamInstitution $institution): void {
            if (blank($institution->slug)) {
                $institution->slug = static::uniqueSlug($institution->name, $institution->id);
            } else {
                $institution->slug = static::uniqueSlug($institution->slug, $institution->id);
            }

            $institution->status ??= self::STATUS_ACTIVE;
        });
    }

    public function programs(): HasMany
    {
        return $this->hasMany(EntranceExamProgram::class);
    }
