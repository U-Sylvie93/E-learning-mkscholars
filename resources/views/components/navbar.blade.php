...
<header class="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
    <nav class="mk-container flex min-h-20 items-center justify-between gap-5 py-3">
        ...
        <div class="hidden items-center gap-1 xl:flex">
            @foreach ($links as $link)
                <a href="{{ route($link['route']) }}" class="rounded-md px-3 py-2 text-sm font-semibold transition mk-focus {{ request()->routeIs($link['route']) || ($link['route'] === 'courses' && request()->routeIs('courses.*')) || ($link['route'] === 'opportunities' && request()->routeIs('opportunities.*')) ? 'bg-mk-goldSoft text-mk-navy' : 'text-slate-600 hover:bg-slate-100 hover:text-mk-navy' }}">
                    {{ $link['label'] }}
                </a>
            @endforeach
        </div>
        ...
        <details class="relative xl:hidden">
            ...
            <div class="absolute right-0 mt-3 w-72 rounded-lg border border-slate-200 bg-white p-2 shadow-soft">
                @foreach ($links as $link)
                    <a href="{{ route($link['route']) }}" class="block rounded-md px-3 py-2 text-sm font-semibold {{ request()->routeIs($link['route']) || ($link['route'] === 'courses' && request()->routeIs('courses.*')) || ($link['route'] === 'opportunities' && request()->routeIs('opportunities.*')) ? 'bg-mk-goldSoft text-mk-navy' : 'text-slate-600 hover:bg-slate-100 hover:text-mk-navy' }}">
                        {{ $link['label'] }}
                    </a>
                @endforeach
