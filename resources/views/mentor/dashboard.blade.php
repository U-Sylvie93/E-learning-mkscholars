<x-dashboard-layout role="mentor" title="Mentor Dashboard" description="MK Scholars mentor dashboard.">
    <div class="space-y-6">
            <x-section-header
                eyebrow="Mentor dashboard"
                title="Guidance and growth"
                description="A protected placeholder for future mentorship sessions, student goals, and opportunity support."
            />

            <x-card highlighted class="min-w-0">
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <x-badge tone="gold">Mentorship workspace</x-badge>
                        <h2 class="mt-4 text-2xl font-extrabold text-mk-navy">Welcome, {{ auth()->user()->name }}</h2>
                        <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Track assigned learners, prepare weekly check-ins, and keep feedback organized in a focused mentor dashboard.</p>
                    </div>
            ...
            <div class="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                ...
                <x-card class="min-w-0">
                    ...
                    <p class="mt-3 text-sm leading-6 text-slate-600">See students assigned to you and the course context for each mentorship relationship.</p>
                    <x-button :href="route('mentor.students')" size="sm" variant="secondary" class="mt-5">View Students</x-button>
                </x-card>
                <x-card class="min-w-0">
                    <x-badge tone="green">Opportunities</x-badge>
