<x-dashboard-layout role="student" title="Mentorship" description="MK Scholars mentorship.">
    <section class="bg-white py-16">
        <div class="mk-container flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <x-section-header
                eyebrow="Student"
                title="Mentorship"
                description="Stay connected with your mentor and weekly check-ins."
            />
            <x-badge tone="gray">{{ $assignments->count() }} mentor assignments</x-badge>
        </div>
    </section>
    ...
    <section class="py-16">
        <div class="mk-container grid gap-8 lg:grid-cols-[1fr_0.8fr]">
            <div class="grid gap-5">
                <h2 class="text-2xl font-extrabold text-mk-navy">Assigned mentors</h2>
                    ...
                    <x-card>
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div>
                                ...
                                <h3 class="mt-4 text-xl font-bold text-mk-navy">{{ $assignment->mentor?->name ?? 'Mentor' }}</h3>
                ...
                @empty
                    <x-card>
                        <h3 class="text-xl font-bold text-mk-navy">No mentor assigned yet</h3>
                        <p class="mt-3 text-sm leading-6 text-slate-600">A mentor will appear here once the MK Scholars team assigns one to support your learning journey.</p>
                    </x-card>
                @endforelse
            </div>
                    ...
                    <div class="mt-5 space-y-3">
                        @forelse ($previousFeedback as $checkIn)
                            <div class="rounded-lg bg-white p-4">
                                ...
                                <p class="mt-2 whitespace-pre-line text-sm leading-6 text-slate-600">{{ $checkIn->mentor_feedback }}</p>
                            </div>
                        @empty
                            <p class="text-sm leading-6 text-slate-600">Mentor feedback will appear here after completed check-ins.</p>
                        @endforelse
                    </div>
                </x-card>
            </div>
